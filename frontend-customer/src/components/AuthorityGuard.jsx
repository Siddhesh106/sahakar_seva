...
export default function AuthorityGuard({ allowedRoles = [], children }) {
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#0a0e19]">
        <div className="flex flex-col items-center gap-3">
          <div className="w-10 h-10 border-4 border-[#8083ff] border-t-transparent rounded-full animate-spin"></div>
          <p className="text-sm font-medium text-[#908fa0]">Verifying sovereign authority...</p>
        </div>
      </div>
    );
  }
