...
export default function Verifications() {
  ...
  return (
    <div className="p-8 max-w-7xl mx-auto space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-slate-900">Worker KYC Verification Queue</h1>
        <p className="text-sm text-slate-500 mt-1">Review identity documents and verify cooperative worker members</p>
      </div>
        ...
        <table className="w-full text-left text-sm text-slate-600">
          <thead className="bg-slate-50 border-b border-slate-200 text-xs font-semibold text-slate-500 uppercase tracking-wider">
            <tr>
              ...
              <th className="px-6 py-4">Doc Type / Number</th>
              ...
              <tr key={w.user_id} className="hover:bg-slate-50/80 transition">
                <td className="px-6 py-4 font-medium text-slate-900">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="font-bold text-slate-800">{w.user?.name}</p>
                      <span className="text-xs text-slate-400">+91 {w.user?.phone}</span>
                    </div>
                  </div>
                </td>
                <td className="px-6 py-4">
                  <div className="flex flex-wrap gap-1">
                    {w.skill_categories?.map((cat) => (
                      <span key={cat} className="px-2 py-0.5 bg-slate-100 text-slate-700 rounded text-[11px] font-semibold capitalize">
                        {cat}
                      </span>
                    ))}
                  </div>
                ...
                <td className="px-6 py-4 text-xs font-mono">
                  <span className="font-semibold uppercase text-slate-700">{w.kyc_doc_type || 'Aadhaar'}</span>
                ...
                <td className="px-6 py-4 text-right space-x-2">
                  ...
                  {w.kyc_status !== 'rejected' && (
                    <button
                      ...
                      className="bg-red-50 hover:bg-red-100 text-red-600 border border-red-200 px-3 py-1.5 rounded-lg text-xs font-semibold transition"
                    >
                      Reject
                    </button>
                  )}
